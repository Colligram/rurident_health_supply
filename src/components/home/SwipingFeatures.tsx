...
export function SwipingFeatures() {
  ...
  return (
    <div className="bg-gradient-to-r from-purple-900 via-pink-900 to-black py-8 md:py-16 relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="container-max relative z-10">
        <div className="text-center mb-8 md:mb-12 px-4">
          ...
          <p className="text-base md:text-lg lg:text-xl text-purple-200 max-w-3xl mx-auto leading-relaxed">
        ...
        <div 
          ...
          className={`flex gap-4 md:gap-6 overflow-x-auto scrollbar-hide pb-4 px-4 md:px-0 ${
        ...
        >
          {features.map((feature, index) => (
            <div
              key={feature.id}
              className={`flex-shrink-0 w-72 md:w-80 bg-white/10 backdrop-blur-sm rounded-2xl p-4 md:p-6 lg:p-8 border border-white/20 transition-all duration-500 transform hover:scale-105 hover:bg-white/20 ${
                index === currentIndex ? 'scale-105 bg-white/20' : ''
              } ${isMobile ? 'snap-start' : ''}`}
              style={{ scrollSnapAlign: isMobile ? 'start' : 'none' }}
            >
              <div className={`w-12 h-12 md:w-16 md:h-16 bg-gradient-to-r ${feature.gradient} rounded-2xl flex items-center justify-center mb-4 md:mb-6 mx-auto transform transition-transform duration-300 hover:rotate-12`}>
                <feature.icon className="w-6 h-6 md:w-8 md:h-8 text-white" />
              </div>
              
              <h3 className="text-lg md:text-xl lg:text-2xl font-bold text-white text-center mb-3 md:mb-4">
                {feature.title}
              </h3>
        ...
        <div className="text-center mt-8 md:mt-12 px-4">
          <button className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-6 md:px-8 py-3 md:py-4 rounded-full text-sm md:text-base lg:text-lg font-semibold hover:from-purple-600 hover:to-pink-600 transition-all duration-300 transform hover:scale-105 shadow-2xl hover:shadow-purple-500/25">
